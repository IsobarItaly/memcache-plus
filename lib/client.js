...
/**
 * metadump() - get all metadata for a given slabs id
 * @param {number|String} slabsId
 * @param {Function} [cb] - The (optional) callback called on completion
 * @returns {Promise}
 */
Client.prototype.metadump = function(slabsId, cb) {
    misc.assert(slabsId, 'Cannot metadump without slabId!');

    return this.run('lru_crawler metadump', [slabsId], cb);
};
