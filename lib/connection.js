...
Connection.prototype.read = function(data) {
    ...
    } else if (data.match(/^key=.+/)) {
        // format is
        // key=<key> exp=<exp> la=<la> cas=<cas> fetch=<fetch> cls=<cls> size=<size>
        var keyData = data.match(/^key=(.+) exp=(-?[0-9]+) la=([0-9]+) cas=([0-9]+) fetch=(.+) cls=([0-9]+) size=([0-9]+)$/);

        var itemKey = decodeURIComponent(keyData[1]);
        var exp = keyData[2];
        var la = keyData[3];
        var cas = keyData[4];
        var fetch = keyData[5];
...
/**
  * 'lru_crawler metadump'() - dump all of the metadata for a given slabs id
  * @param {number} slabsId
  */
 Connection.prototype['lru_crawler metadump'] = function(slabsId) {
    var deferred = misc.defer(slabsId);
    deferred.type = 'items';
    this.commandBuffer = this.commandBuffer.push(deferred);

    this.write(util.format('lru_crawler metadump %s', slabsId));
